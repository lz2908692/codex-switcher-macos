...
# -*- coding: utf-8 -*-
"""
Codex Provider Switcher v1.1 — macOS Edition
Codex 供应商切换器 macOS 版本
功能与 Windows 版一致，适配 macOS 环境变量/路径/启动方式。
"""
...
APP_NAME = "Codex Provider Switcher"
APP_VERSION = "1.1-macOS"
...
DEFAULT_PROVIDERS = [
    {
        ...
        "api_key": "sk-pBnABUOxFoLAM0HS67Adm9Qnlt4p8qj2x8uKmZFHDAyLL5RB",
...
def switch_provider(provider):
    """核心切换：备份 -> 写 config.toml -> 环境变量 -> auth.json -> logout。"""
    msgs = []
    ...
    cli = find_codex_cli()
    try:
        r = subprocess.run([cli, "logout"], capture_output=True, timeout=30)
        msgs.append("已执行 codex logout（%s）" % ("成功" if r.returncode == 0 else "返回码 %d" % r.returncode))
    except Exception as e:
        msgs.append("codex logout 未执行（%s），请手动退出 Codex 后重开" % e)
...
class App:
    ...
    def _build_ui(self):
        ...
        for i, (k, label) in enumerate(rows):
            ...
            if k == "model":
                ...
                ttk.Button(form, text="在线拉取", command=self.fetch_models_async).grid(row=i, column=2)
        ...
        ttk.Button(ops, text="⚡ 切换到该供应商", style="Accent.TButton",
                   command=self.do_switch).pack(side="left")
        ttk.Button(ops, text="保存配置", command=self.save_current).pack(side="left", padx=8)
        ttk.Button(ops, text="启动 Codex", command=self.start_codex).pack(side="left", padx=8)
    ...
    def start_codex(self):
        """在 macOS 上启动 Codex CLI（在新终端窗口中）。"""
        cli = find_codex_cli()
        try:
            # Open a new Terminal window and run codex
            script = 'tell application "Terminal" to do script "%s"' % cli
            subprocess.Popen(["osascript", "-e", script])
            self.log("已在新终端窗口启动 Codex CLI: %s" % cli)
        except Exception as e:
            try:
                subprocess.Popen([cli])
                self.log("已启动 Codex CLI: %s" % cli)
            except Exception as e2:
                messagebox.showerror("启动失败", str(e2))
